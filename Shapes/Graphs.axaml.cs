...
public partial class TimeGraphs : Window
{
    ...
    public TimeGraphs()
    {
        ...
        {
            for(int j = 0; j < 10; j++)
                shapes.Add(new Circle(rnd.Next(-1000, 1001), rnd.Next(-1000, 1001)));
            
            graphs[0].Add(CC.CountTime(Methods.ByDefenition, shapes.ToList()));
            graphs[1].Add(CC.CountTime(Methods.ByGraham, shapes.ToList()));
            graphs[2].Add(CC.CountTime(Methods.ByAndrew, shapes.ToList()));
        }
